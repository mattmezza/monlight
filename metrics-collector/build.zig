...
pub fn build(b: *std.Build) void {
    ...
    const aggregation_tests = b.addTest(.{
        .root_source_file = b.path("src/aggregation.zig"),
        .target = target,
        .optimize = optimize,
    });
    aggregation_tests.root_module.addImport("sqlite", sqlite_mod);
    aggregation_tests.linkSystemLibrary("sqlite3");
    aggregation_tests.linkLibC();
    ...
    const metrics_query_tests = b.addTest(.{
        .root_source_file = b.path("src/metrics_query.zig"),
        .target = target,
        .optimize = optimize,
    });
    metrics_query_tests.root_module.addImport("sqlite", sqlite_mod);
    metrics_query_tests.linkSystemLibrary("sqlite3");
    metrics_query_tests.linkLibC();
