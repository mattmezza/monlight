...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{ .name = "metrics-collector", .root_module = exe_mod });
    ...
    const run_step = b.step("run", "Run the metrics collector server");
    ...
    // Test step — tests for aggregation.zig
    const aggregation_test_mod = b.createModule(.{
        .root_source_file = b.path("src/aggregation.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "sqlite", .module = sqlite_mod },
    ...
    aggregation_test_mod.linkSystemLibrary("sqlite3", .{});
    aggregation_test_mod.link_libc = true;

    const aggregation_tests = b.addTest(.{ .root_module = aggregation_test_mod });
    const run_aggregation_tests = b.addRunArtifact(aggregation_tests);

    // Test step — tests for retention.zig
    ...
    // Test step — tests for metrics_query.zig
    const metrics_query_test_mod = b.createModule(.{
        .root_source_file = b.path("src/metrics_query.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "sqlite", .module = sqlite_mod },
    ...
    metrics_query_test_mod.linkSystemLibrary("sqlite3", .{});
    metrics_query_test_mod.link_libc = true;

    const metrics_query_tests = b.addTest(.{ .root_module = metrics_query_test_mod });
    const run_metrics_query_tests = b.addRunArtifact(metrics_query_tests);
    ...
    test_step.dependOn(&run_aggregation_tests.step);
    test_step.dependOn(&run_retention_tests.step);
    test_step.dependOn(&run_metrics_query_tests.step);
